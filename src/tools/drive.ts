...
export function registerDriveTools(server: McpServer): void {
  ...
  server.tool(
    "drive_search",
    "Searches Google Drive files using drive.files.list with a query built from name, MIME type, modification date, and owner filters; excludes trashed files automatically and supports pagination. Use when the user asks to find a file or folder by name or type in Drive. Use when discovering file IDs before operating on files with other drive_ tools. Do not use when: retrieving metadata for a known file ID (use drive_get_info); listing spreadsheets the user owns or has access to without extra filters (use sheets_list); reading the contents of a Google Doc (use docs_get_text after resolving the ID); downloading file bytes (use drive_download); listing who has access to a known file (use drive_list_permissions); finding files to trash or untrash (use drive_search then drive_trash - this tool is the discovery half of that pair); searching inside file content rather than metadata (not supported - use the Google Drive web UI fulltext search or export + grep). Returns: one block per file with name, id, type, modified timestamp, and url; blocks separated by blank lines. Appends 'nextPageToken: ...' when more pages exist. Parameters: - query: text to match in file names, e.g. 'budget 2025' - mimeType: sheet, doc, folder, slide, form, or a full MIME type string - modifiedAfter: ISO date string, e.g. '2024-01-01' - pageSize: results per page (default 20, max 100).",
    {
