...
export function registerChartTools(server: McpServer): void {
  ...
  server.tool(
    "sheets_create_chart",
    "Creates an embedded chart in a Google Sheet using spreadsheets.batchUpdate with addChart; the first column of the data range is used as the X-axis domain and remaining columns as series. Use when the user asks to visualize data in a sheet as a bar, line, pie, scatter, area, column, or combo chart. Use when adding a chart to a dashboard sheet after building it with sheets_build_dashboard. Do not use when: removing an existing chart (use sheets_delete_chart with the returned chartId); building a full multi-chart dashboard from scratch (use sheets_build_dashboard); building a full formatted single sheet including optional charts in one declarative call (use sheets_build_sheet); highlighting cells based on rules rather than drawing a chart (use sheets_add_conditional_format or sheets_add_conditional_formats); applying static cell colors or borders for visual emphasis (use sheets_format_cells or sheets_apply_formats). Returns: 'Created {chart_type} chart (chartId: {chartId}) in sheet \"{sheet}\"'. The returned chartId is required by sheets_delete_chart. Parameters: - chart_type: one of BAR, LINE, PIE, SCATTER, AREA, COLUMN, COMBO - data_range: A1 notation range where the first column is X-axis labels, e.g. 'Sheet1!A1:C20' - sheet: tab name or ID where the chart will be embedded.",
    {
  ...
  server.tool(
    "sheets_delete_chart",
    "Deletes an embedded chart from a spreadsheet using spreadsheets.batchUpdate with deleteEmbeddedObject; the chart is removed but the underlying data range is not affected. Use when the user asks to remove a chart that is no longer needed. Use when replacing a chart by deleting the old one and calling sheets_create_chart with updated parameters. Do not use when: creating a new chart (use sheets_create_chart); rebuilding a full dashboard including chart teardown (use sheets_build_dashboard); removing a conditional formatting rule (use sheets_add_conditional_formats with an empty rule list or clear the rule via sheets_batch_update); clearing data validation (use sheets_clear_validation); deleting the whole sheet tab that contains the chart (use sheets_delete_sheet); deleting the entire spreadsheet file (use sheets_delete or drive_trash). Returns: 'Deleted chart {chart_id}'. Parameters: - chart_id: numeric chart ID returned by sheets_create_chart; obtain from the sheet if the ID is not known by inspecting the spreadsheet JSON.",
    {
      spreadsheet_id: z.string().describe("sheet ID from the URL (the token between /d/ and /edit) or the full URL"),
